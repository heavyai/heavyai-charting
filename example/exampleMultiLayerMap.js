...
document.addEventListener("DOMContentLoaded", () => {
  ...
  new DbCon()
    ...
    .connect((error, con) => {
      ...
      // table
      const tableName1 = ["contributions_donotmodify", "zipcodes_2017"];
      const table1Joins = [{
      ...
      // first layer
      const crossFilter = crossfilter.crossfilter(con, tableName1, table1Joins).then((cf1) => {

        // second layer
        const crossFilter = crossfilter.crossfilter(con, tableName2).then((cf2) => {

          // third layer
          const crossFilter = crossfilter.crossfilter(con, tableName3).then((cf3) => {
  ...
  function createPointMap(polycfLayer1, pointcfLayer2, pointcfLayer3, con) {
    ...
    const dataCount1 = dc.countWidget(".data-count1")
                       .dimension(pointcfLayer2)
                       .group(countGroup1);

    const countGroup2 = pointcfLayer3.groupAll();
    const dataCount2 = dc.countWidget(".data-count2")
    ...
    const polyDim1 = polycfLayer1.dimension("contributions_donotmodify.amount");
    ...
    const pointMapDim2 = pointcfLayer2.dimension(null).projectOn(["conv_4326_900913_x(lon) as x", "conv_4326_900913_y(lat) as y", "lang as color", "followers as size"]);
    ...
    const xDim2 = pointcfLayer2.dimension("lon");
    const yDim2 = pointcfLayer2.dimension("lat");
    ...
    const sizeScaleLayer2 = d3.scale.linear().domain([0,5000]).range([2,12]).clamp(true);
    ...
    const langDomain = ['en', 'pt', 'es', 'in', 'und', 'ja', 'tr', 'fr', 'tl', 'ru', 'ar']
    const langColors = ["#27aeef", "#ea5545", "#87bc45", "#b33dc6", "#f46a9b", "#ede15b", "#bdcf32", "#ef9b20", "#4db6ac", "#edbf33", "#7c4dff"]

    const layer2ColorScale = d3.scale.ordinal().domain(langDomain).range(langColors);
    ...
    const pointMapDim3 = pointcfLayer3.dimension(null).projectOn(["conv_4326_900913_x(lon) as x", "conv_4326_900913_y(lat) as y", "recipient_party as color"]);

    // we need separate dimensions for the x and y coordinates for point layers.
    // A filter is applied to these dimensions under the hood so that we only
    // render points that are within the view.
    const xDim3 = pointcfLayer3.dimension("lon");
    const yDim3 = pointcfLayer3.dimension("lat");
    ...
    const dynamicSizeScale = d3.scale.sqrt().domain([100000,0]).range([1.0,7.0]).clamp(true)
    ...
    const layer3ColorScale = d3.scale.ordinal().domain(["D", "R"]).range(["blue", "red"]);
    ...
    const pointMapChart = dc.rasterChart(parent, true) // create a raster chart. true indicates a pointmap
    ...
    pointMapChart.init().then(() => {
      ...
      pointMapChart.map().on('movestart', () => {
      ...
      pointMapChart.map().on('moveend', (event) => {
      ...
      pointMapChart.map().on('mousemove', (event) => {
      ...
      function displayPopupWithData (event) {
        if (event.point) {
          ...
          pointMapChart.getClosestResult(event.point, (closestPointResult) => {
