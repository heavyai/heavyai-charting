...
export default function mapMixin (_chart, chartDivId, _mapboxgl, mixinDraw = true, useMapboxDraw = true) {
  ...
  _chart.getDataRenderBounds = function () {
    const bounds = hasAppliedInitialBounds || !initialBounds ? _map.getBounds() : initialBounds
  ...
  _chart.on("postRender", () => {
    ...
    _hasRendered = true
    const boundsChanged = initialBounds && !boundsRoughlyEqual(initialBounds, _map.getBounds())
  ...
  _chart.initialBounds = function (bounds) {
    ...
    const latMaxSafe = bounds.latMax < LATMAX ? bounds.latMax : LATMAX
    const latMinSafe = bounds.latMin > LATMIN ? bounds.latMin : LATMIN
    const lonMaxSafe = bounds.lonMax < LONMAX ? bounds.lonMax : LONMAX
    const lonMinSafe = bounds.lonMin > LONMIN ? bounds.lonMin : LONMIN

    const sw = new _mapboxgl.LngLat(lonMinSafe, latMinSafe)
    const ne = new _mapboxgl.LngLat(lonMaxSafe, latMaxSafe)
