import {constants, decrementSampledCount, incrementSampledCount, transition} from "../core/core"
...
/* OVERRIDE EXTEND ----------------------------------------------------------*/
  ...
  _chart.sampling = function (setting) { // setting should be true or false
    ...
    if (setting && !_sampling) // if wasn't sampling
          { incrementSampledCount() } else if (!setting && _sampling) { decrementSampledCount() }
    _sampling = setting
    if (_sampling == false) { _chart.dimension().samplingRatio(null) } // unset sampling
    return _chart
  }
  ...
  _chart.onClick = function (d) {
    if (_chart.bounds == null) { return }
    const xPixelScale = 1.0 / (_chart.bounds[1][0] - _chart.bounds[0][0]) * _chart.width()
    const yPixelScale = 1.0 / (_chart.bounds[1][1] - _chart.bounds[0][1]) * _chart.height()
    const mapCoords = conv4326To900913([d.x, d.y])
    const pixelPos = {x: (mapCoords[0] - _chart.bounds[0][0]) * xPixelScale, y: _chart.height() - (mapCoords[1] - _chart.bounds[0][1]) * yPixelScale}
...
/* OVERRIDE EXTEND ----------------------------------------------------------*/
  function mapDataToPoints (data) {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      const yPixel = _chart.height() - (coordTrans[1] - _chart.bounds[0][1]) * yPixelScale
  ...
  _chart.plotData = function () {
    ...
    if (!_g) { initOverlayG() }
    const bubbleG = _g.selectAll("g." + BUBBLE_NODE_CLASS).data(_chart.savedData, (d) => d.key)
    ...
    bubbleG.enter().append("g")
            ...
            .attr("r", (d) => _chart.scaleRadius ? _chart.bubbleR(d) : _chart.radiusValueAccessor()(d))
...
/* OVERRIDE -----------------------------------------------------------------*/
    ...
    const bubbleG = _g.selectAll("g." + BUBBLE_NODE_CLASS)
