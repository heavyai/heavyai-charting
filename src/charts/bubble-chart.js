...
export default function bubbleChart(parent, chartGroup) {
  ...
  _chart.hideOverlappedLabels = function() {
    ...
    nodes
      .classed(
        "hide-label",
        d => _chart.bubbleR(d) < _chart.minRadiusWithLabel()
      )
      .each(function(d, i) {
        const a = this;
  ...
  function isNodeOverlapped(d1, d2) {
    const dist = Math.sqrt(
      (d1.xPixel - d2.xPixel) * (d1.xPixel - d2.xPixel) +
        (d1.yPixel - d2.yPixel) * (d1.yPixel - d2.yPixel)
    );
  ...
  _chart.showPopup = function(d, i) {
    ...
    popupBox
      ...
      .style(
        "border-radius",
        "0 0 " + _chart.bubbleR(d) + "px " + _chart.bubbleR(d) + "px"
      )
      .on("click", () => {
        _chart.onClick(d);
      })
    ...
    headerItems
      ...
      .on("mouseenter", function() {
        d3.select(this).style("transform", function() {
          ...
          elm
            .style("transition-duration", dist * 0.05 + "s")
    ...
    popup.classed(
      "js-showPopup popup-scrollable delay-pointer scatter-plot-popup",
      true
    );
  ...
  function renderPopupRow(d) {
    let str = `
            <td>
                <div class="table-dim">
                    <div class="table-legend" style="background:${_chart.getColor(
                      d
                    )}"></div>
    ...
    for (let i = 1; i < _popupHeader.length; i++) {
      if (_popupHeader[i].alias) {
        str =
          str +
          ("<td>" + utils.formatValue(d[_popupHeader[i].alias]) + "</td>");
      }
    }
    return str;
  }
  ...
  function positionPopup(d, e) {
    ...
    const popup = _chart.popup().style("transform", function() {
      const popupWidth =
        d3
          .select(this)
    ...
    popup
      ...
      .classed("popdown", function() {
        return (
          popup.node().getBoundingClientRect().top - 76 <
          d3
            .select(this)
            .node()
  ...
  function renderNodes(bubbleG) {
    ...
    bubbleGEnter
      .append("circle")
      .attr("class", (d, i) => _chart.BUBBLE_CLASS + " _" + i)
