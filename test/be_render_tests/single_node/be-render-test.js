...
const zipcode_tests = require('./test_configs/zipcode-poly-render-test');
...
function omnisciDisconnect() {
  return new Promise((resolve, reject) => {
    if (conn) {
      conn.disconnect((error, this_conn) => {
        if (error) {
          reject(error);
        }
        conn = null;
...
function handleTestConfig(test_config) {
  it(test_config.test_desc, async function() {
    ...
    if (command === "renderVega") {
      await handleRenderVega(args, expectation)
    }
  });
}
