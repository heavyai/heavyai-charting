...
module.exports = function(test_collection, expect) {
    ...
    data: [
      {
        ...
        format: 'polys',
        sql:
          'SELECT nyc_buildings.rowid, nyc_buildings.bbl as key0, avg(heightroof) as color FROM nyc_buildings GROUP BY nyc_buildings.rowid, key0'
      }
    ],
    projections: [
      {
  ...
  const test_grp = new OmniSciServerTestGroup({
    ...
    before: function(test_state) {
      return test_state.server_connection.clearGpuMemoryAsync();
    }
  });
