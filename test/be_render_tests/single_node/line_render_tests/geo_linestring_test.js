...
module.exports = function(test_collection, expect) {
    ...
    data: [
      {
        ...
        sql:
          'SELECT rowid, mapd_geo as points FROM north_america_rivers WHERE (ST_XMax(north_america_rivers.mapd_geo) >= -179.99999999999886 AND ST_XMin(north_america_rivers.mapd_geo) <= -27.743704049260515 AND ST_YMax(north_america_rivers.mapd_geo) >= 9.429808031730957 AND ST_YMin(north_america_rivers.mapd_geo) <= 82.20270263522823)'
      }
    ],
    scales: [],
    projections: [
  ...
  vega.data[0].sql = `SELECT rowid, mapd_geo FROM north_america_rivers WHERE (ST_XMax(mapd_geo) >= ${empty_bounds[0]} AND ST_XMin(mapd_geo) <= ${empty_bounds[1]} AND ST_YMax(mapd_geo) >= ${empty_bounds[2]} AND ST_YMin(mapd_geo) <= ${empty_bounds[3]})`;
