...
module.exports = function(test_collection, expect, is_distributed) {
  ...
  const legacy_test_grp = test_grp.createTestSubGroup("legacy_poly_hittest_test", {
    test_description: "Tests hit-testing legacy poly queries."
  });

  const latest_test_grp = test_grp.createTestSubGroup("latest_poly_hittest_test", {
    test_description: "Tests hit-testing latest poly queries."
  });

  function build_tests(test_grp, use_latest_query) {
    if (!is_distributed || use_latest_query) {
      // prettier-ignore
      const vega = {
        "width": 1183,
        "height": 1059,
        "data": [
          {
            "name": "zipcodes",
            "format": "polys",
            "sql": use_latest_query ? "SELECT omnisci_geo FROM zipcodes_2017" : "SELECT rowid from zipcodes_2017"
          }
        ],
        "projections": [
          {
      ...
      let prev_test_name = test_grp.addTest(
        ...
        ],
        {
          pending: use_latest_query
            ? "Pending an fix for an odd shader compilation error found on 05/05/19 (no jira issue yet)"
            : false
        }
      );
      ...
      test_grp.addTest(
        ...
        [
          new RenderVegaTest(
            (() => {
              vega.data[0].sql = `SELECT ${use_latest_query
                ? "omnisci_geo, "
                : ""}rowid, ALAND10 as area from zipcodes_2017`;
              return vega;
            })(),
      ...
      test_grp.addTest(
        ...
        [
          new RenderVegaTest(
            (() => {
              vega.data[0].sql = `SELECT ${use_latest_query ? "omnisci_geo, " : ""}rowid, ZCTA5CE10 from zipcodes_2017`;
              return vega;
            })(),
            (result) => expect(result).to.matchGoldenImage("poly_hittest_test_01.png")
          ),
      ...
      test_grp.addTest(
        ...
        [
          new RenderVegaTest(
            (() => {
              vega.data[0].sql = `SELECT rowid, ZCTA5CE10 as key0, ${use_latest_query
                ? "SAMPLE(omnisci_geo) as omnisci_geo, "
                : ""}avg(ALAND10) as color FROM zipcodes_2017 GROUP BY rowid, key0 HAVING key0 ilike '941__' ORDER BY rowid`;
              return vega;
            })(),
        ...
        {
          pending: use_latest_query ? "Pending support for geo-spatial columns in temporary tables." : false
        }
      );
      ...
      test_grp.addTest(
        ...
        [
          new RenderVegaTest(
            (() => {
              vega.data[0].sql = `SELECT zipcodes_2017.rowid, contributions.contributor_zipcode as key0, count(*) as color${use_latest_query
                ? ", SAMPLE(omnisci_geo) as omnisci_geo "
                : ""} FROM contributions, zipcodes_2017 WHERE (contributions.contributor_zipcode = zipcodes_2017.ZCTA5CE10) GROUP BY zipcodes_2017.rowid, key0`;
              return vega;
            })(),
            (result) => expect(result).to.matchGoldenImage("poly_hittest_test_01.png")
          ),
        ...
        {
          pending:
            use_latest_query && is_distributed ? "Pending support for geo-spatial columns in temporary tables." : false
        }
      );
    }
  }
