...
describe("Map Draw Mixin", () => {
  let oldRedrawAllAsync
  before(() => {
    oldRedrawAllAsync = dc.redrawAllAsync
    dc.redrawAllAsync = chai.spy()
  })
  after(() => {
    dc.redrawAllAsync = oldRedrawAllAsync
  })
  describe("addDrawControl", () => {
    xit('should add draw control to map', () => {
  ...
  describe("drawEventHandler", () => {
    xit('should clear filter if there are no features', () => {
    ...
    xit('should create SQL filter from features', () => {
      const features = [
        {
    ...
    xit('should call redrawAllAsync', () => {
      expect(dc.redrawAllAsync).to.have.been.called()
    })
  })
  describe("changeDrawMode", () => {
    xit('should set draw mode true if mode is draw_polygon', () => {
      const filter = chai.spy()
      const chart = createChartWithDrawContext({
        getAll: () => ({ features: [] }),
    ...
    xit('should set draw mode false if mode is not draw_polygon and none are selected', () => {
      const filter = chai.spy()
      const chart = createChartWithDrawContext({
        getAll: () => ({ features: [] }),
        getSelected: () => ({ features: [] }),
